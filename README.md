...
AI 检测工作台支持自动模式。食堂工作人员开启时填写风险阈值，例如 `60`，浏览器会常驻打开摄像头；当画面检测到餐盘/饭菜并稳定数秒后，系统自动截图并调用 Kimi 分析。AI 风险分数低于阈值时，结果展示几秒后自动等待下一份餐盘；分数高于或等于阈值时，系统会暂停，必须点击“人工确认，继续下一盘”后才会进入下一轮。

自动模式需要浏览器摄像头权限，并且通常需要在 HTTPS 环境下运行；Render 线上地址满足这个要求。

## 仪表板数据

总览仪表板不再使用固定演示数字。今日检测数、高风险提醒、过敏原命中和最近 7 天趋势都会根据实际完成的 AI 检测记录自动变化。

每次完成检测后，系统也会把当前餐盘照片压缩成历史记录缩略图一起保存。管理者在“最近检测记录”或“检测记录”表格中可以点击小图查看餐盘图片，方便回看当时 AI 判断对应的是哪一份餐盘。
...
如果你的 `scan_records` 表之前已经创建过，也需要重新运行一次 `supabase-schema.sql`，或单独执行下面这句，为历史记录图片新增字段：
